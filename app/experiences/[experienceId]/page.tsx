...
export default async function ClientExperiencePage({
  ...
  if (authContext.accessLevel === "admin") {
    try {
      ...
      return <AccessDenied title="Coach dashboard temporarily unavailable" message="Your administrator access is confirmed, but the workspace could not be resolved. Refresh inside Whop or open FITz from your business dashboard." />;
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-slate-100 font-sans">
      <ClientPortal
        ...
        coachingEnabled={slots.selectedIds.includes(client.id)}
      />
    </div>
  );
}
